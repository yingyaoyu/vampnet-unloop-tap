...
class VampNet(at.ml.BaseModel):
    ...
    def _empty_ctrls(self, embedding: Tensor):
        ctrls = {}
        ctrl_masks = {}
        batch_size, _, n_frames = embedding.shape
        for key, dim in self.ctrl_dims.items():
            ctrls[key] = torch.zeros(
                batch_size,
                dim,
                n_frames,
    ...
    def forward(self, x, ctrls=None, ctrl_masks=None, return_activations: bool = False):
        ...
        if self.ctrl_dims is not None:
            if ctrls is None:
                ctrls, ctrl_masks = self._empty_ctrls(x)
            elif ctrl_masks is None:
                ctrl_masks = {
                    key: torch.ones(
                        x.shape[0],
                        x.shape[-1],
    ...
    def generate(
        ...
        for i in range(sampling_steps):
            ...
            logits = self.forward(latents, ctrls=ctrls, ctrl_masks=ctrl_masks) # b, prob, seq
