...
from vampnet.motion import MotionFeatureController, MotionSidecarDataset
...
def build_motion_controller(
    ...
    if not enabled:
        return None
    return MotionFeatureController(
...
def build_datasets(args, sample_rate: int):
    ...
    if args.get("motion.enabled", False) and args.get("motion.sidecar_dir") is not None:
        train_data = MotionSidecarDataset(
            train_data,
            sidecar_dir=args.get("motion.sidecar_dir"),
            sidecar_suffix=args.get("motion.sidecar_suffix", ".motion.npz"),
            audio_path_key=args.get("motion.audio_path_key"),
            motion_path_key=args.get("motion.path_key", "motion_path"),
        )
        val_data = MotionSidecarDataset(
            val_data,
            sidecar_dir=args.get("motion.sidecar_dir"),
            sidecar_suffix=args.get("motion.sidecar_suffix", ".motion.npz"),
            audio_path_key=args.get("motion.audio_path_key"),
...
def get_controls(state, sig: at.AudioSignal, batch: Optional[dict] = None, n_tokens: Optional[int] = None):
    # get controls
    n_batch = sig.samples.shape[0]  
    if state.controller is not None:
        device = getattr(state, "device", sig.samples.device)
        try:
            extracted = state.controller.extract(
                sig,
                batch=batch,
                n_tokens=n_tokens,
                device=device,
            ...
            extracted = state.controller.extract(sig)

        if isinstance(extracted, tuple):
            ctrls, available_masks = extracted
        else:
            ctrls = extracted
        ...
        ctrl_masks = state.controller.random_mask(
            ctrls, 
            r=state.rng.draw(n_batch)[:, 0].to(device)
        )
        if available_masks is not None:
            ctrl_masks = {
                k: ctrl_masks[k] * available_masks[k].to(ctrl_masks[k].device)
                for k in ctrl_masks
            }
    else:
        ctrls = None
...
def train_loop(state: State, batch: dict, accel: Accelerator):
    ...
    with accel.autocast():
        ...
        # get controls
        ctrls, ctrl_masks = get_controls(state, signal, batch=batch, n_tokens=z.shape[-1])
        ...
        with accel.autocast(dtype=dtype):
            z_hat = state.model(z_mask_latent, ctrls=ctrls, ctrl_masks=ctrl_masks)
...
def load(
    ...
    controller = build_motion_controller()
    if controller is not None:
        model_ctrl_dims = getattr(accel.unwrap(model), "ctrl_dims", None)
        if model_ctrl_dims is None:
            tracker.print(
                "WARNING: motion.enabled is true, but VampNet.ctrl_dims is unset. "
                "Set `VampNet.ctrl_dims: {motion: 3}` to train motion conditioning."
            )
        elif controller.ctrl_dims != model_ctrl_dims:
            tracker.print(
                f"WARNING: controller dims {controller.ctrl_dims} do not match "
                f"model dims {model_ctrl_dims}."
            )
