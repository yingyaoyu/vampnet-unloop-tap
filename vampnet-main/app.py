...
# load the init model (compute once — used for the Gradio dropdown)
...
with gr.Blocks() as demo:
    ...
    harp_input_audio = gr.Audio(
        label="input audio (HARP)",
        type="filepath",
        visible=False,
    )
    harp_output_audio = gr.Audio(
        label="output audio (HARP)",
        type="filepath",
        interactive=False,
        visible=False,
    )
    harp_output_labels = gr.JSON(label="Output Labels", visible=False)
    ...
    app = build_endpoint(
        ...
        output_components=[harp_output_audio, harp_output_labels],
        process_fn=harp_vamp,
    )

    @demo.load(inputs=None, outputs=model_choice)
    def _ensure_model_dropdown():
        return gr.update(choices=_AVAILABLE_MODELS, value=init_model_choice)
